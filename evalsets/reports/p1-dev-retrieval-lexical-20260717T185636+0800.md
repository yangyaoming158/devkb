...
> 运行：2026-07-17T18:56:36+08:00 ｜ devkb commit：`30c561c2a6e769f569c24558accc71d2376a402c`
...
每题 5 次计时、共 85 个样本：P50=17.6ms，P95=78.3ms，max=129.4ms。
...
```text
Limit  (cost=982.72..982.74 rows=10 width=1440) (actual time=19.878..19.881 rows=10 loops=1)
  Buffers: shared hit=9251
  ->  Sort  (cost=982.72..985.25 rows=1013 width=1440) (actual time=19.878..19.879 rows=10 loops=1)
        ...
        Buffers: shared hit=9251
        ->  Hash Join  (cost=27.42..960.83 rows=1013 width=1440) (actual time=0.282..19.574 rows=636 loops=1)
              Hash Cond: (chunks.document_id = documents.id)
              Buffers: shared hit=9251
              ->  Seq Scan on chunks  (cost=0.00..928.19 rows=1013 width=1356) (actual time=0.072..9.521 rows=636 loops=1)
              ...
              ->  Hash  (cost=21.71..21.71 rows=457 width=96) (actual time=0.136..0.136 rows=450 loops=1)
                    Buckets: 1024  Batches: 1  Memory Usage: 65kB
                    Buffers: shared hit=16
                    ->  Seq Scan on documents  (cost=0.00..21.71 rows=457 width=96) (actual time=0.003..0.077 rows=450 loops=1)
                          Filter: ((status)::text = 'active'::text)
                          Buffers: shared hit=16
Planning:
  Buffers: shared hit=7
Planning Time: 0.251 ms
Execution Time: 19.894 ms
```

### `创建订单接口是怎么防止用户重复提交的？`（GIN 命中：否）

```text
Limit  (cost=970.70..970.73 rows=10 width=1440) (actual time=11.580..11.583 rows=10 loops=1)
  Buffers: shared hit=8387
  ->  Sort  (cost=970.70..972.11 rows=564 width=1440) (actual time=11.579..11.581 rows=10 loops=1)
        ...
        Buffers: shared hit=8387
        ->  Hash Join  (cost=27.42..958.52 rows=564 width=1440) (actual time=0.648..11.411 rows=284 loops=1)
              Hash Cond: (chunks.document_id = documents.id)
              Buffers: shared hit=8387
              ->  Seq Scan on chunks  (cost=0.00..928.19 rows=564 width=1356) (actual time=0.486..8.271 rows=284 loops=1)
                    ...
                    Buffers: shared hit=7604
              ->  Hash  (cost=21.71..21.71 rows=457 width=96) (actual time=0.142..0.143 rows=450 loops=1)
                    Buckets: 1024  Batches: 1  Memory Usage: 65kB
                    Buffers: shared hit=16
                    ->  Seq Scan on documents  (cost=0.00..21.71 rows=457 width=96) (actual time=0.002..0.109 rows=450 loops=1)
                          Filter: ((status)::text = 'active'::text)
                          Buffers: shared hit=16
Planning:
  Buffers: shared hit=7
...
```text
Limit  (cost=401.58..401.60 rows=10 width=1440) (actual time=0.611..0.614 rows=10 loops=1)
  Buffers: shared hit=132
  ->  Sort  (cost=401.58..401.64 rows=24 width=1440) (actual time=0.611..0.613 rows=10 loops=1)
        Sort Key: (ts_rank_cd(chunks.search_tsv, '''40901'''::tsquery)) DESC, chunks.id
        Sort Method: quicksort  Memory: 53kB
        Buffers: shared hit=132
        ->  Hash Join  (cost=316.63..401.06 rows=24 width=1440) (actual time=0.227..0.598 rows=13 loops=1)
              Hash Cond: (chunks.document_id = documents.id)
              Buffers: shared hit=132
              ->  Bitmap Heap Scan on chunks  (cost=289.20..373.51 rows=24 width=1356) (actual time=0.124..0.140 rows=13 loops=1)
                    Recheck Cond: (search_tsv @@ '''40901'''::tsquery)
                    Filter: (project_id = '46eebe95-e041-4f2f-85bc-4b2ed0d8bceb'::uuid)
                    Heap Blocks: exact=13
                    Buffers: shared hit=81
                    ->  Bitmap Index Scan on ix_chunks_search_tsv  (cost=0.00..289.20 rows=24 width=0) (actual time=0.120..0.120 rows=13 loops=1)
                          Index Cond: (search_tsv @@ '''40901'''::tsquery)
                          Buffers: shared hit=68
              ->  Hash  (cost=21.71..21.71 rows=457 width=96) (actual time=0.089..0.090 rows=450 loops=1)
                    Buckets: 1024  Batches: 1  Memory Usage: 65kB
                    Buffers: shared hit=16
                    ->  Seq Scan on documents  (cost=0.00..21.71 rows=457 width=96) (actual time=0.003..0.059 rows=450 loops=1)
                          Filter: ((status)::text = 'active'::text)
                          Buffers: shared hit=16
Planning:
  Buffers: shared hit=7
...
- **机器实验**：诊断列显示多数未命中题的相关块连 top-100 都未进入。另在同一快照上对照了 ts_rank_cd 归一化 flag 1/4/32 与查询侧丢弃单字 CJK token 共 6 个排序变体，R@10 全部不变（0.059）——这不是排序调参能解决的问题。
