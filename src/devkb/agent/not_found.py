...
# 上面**两张表**的并集，**只用于降级、永不用于断言**（T31.2R-d）。
#
# 那两张表各自是「用来产生断言」的表：命中即判定「这是仓库级否定 / 这是覆盖缺口」。
# 那种用法要求表是穷尽的，而它证不出来——T31.2R-b 正因如此把
# `zero_absence_assertion_on_known_paths` 由硬键降为报告项。
#
# 本常量反过来用：它只用来**否决 `full` 终态**（`nodes.finalize_consistency`）。
# 误报的代价是「本可以 full 的题变成 partial」，方向保守，**不产生任何虚假断言**；
# 漏报（措辞全在表外）由 `test_agent_graph.py` 的 T31.2R-d U3 当场登记为已知边界。
# 两者都不需要「这张表是穷尽的」这个前提。
#
# **`_SOFT_NEGATION` 不并进来。**首版并了，结果 `test_all_aspects_cited_still_reaches_full`
# 当场转红：它的正文「NO_ANSWER 由**无** hit 触发」只命中一个字 `无`，而那句话根本
# 不是否定断言。这四个标记匹配的是**字**不是断言（`无 hit`、`未知`、`缺省` 在中文技术
# 行文里到处都是），本模块既有语义边界也只在**穷举量词同现**时才把它们当否定看
# （见 `_SOFT_NEGATION` 的定义注释与 `_looks_like_absence` 的用法）。收窄后 e06 仍被
# `未出现` 抓住，冻结用例恢复通过。
#
# **必不穷尽**，且**不得**用它去判定「这句话是不是仓库级断言」——U9 用位置白名单
# 把这条纪律变成机器可判定的约束（生产源码里只许出现三处）。
