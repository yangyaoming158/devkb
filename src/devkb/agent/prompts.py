...
GENERATE_SYSTEM = (
    ...
    # T26.1（RT-18 / 案例七）：开关不能只认 requested_mode——sufficiency 为 sufficient 时
    # 它是 "full"，而案例七恰好走这条路径（正文全称断言与自述缺口并存）。故以模型自己
    # 是否写 not_found 为主开关，两者取或。
    "当你在 not_found 写入任何条目、或 requested_mode 为 partial 时，正文结论必须限定在"
    "**已引用证据实际覆盖的资源与操作**；对未检查的对象不得使用“任何/所有/全部/一切/"
    "均/都/一律”等全称表述下结论。"
    "not_found 只描述**当前证据范围内**的缺口，用条件式措辞（如“当前证据未覆盖…”）；"
    "不得断言仓库或项目中不存在某文件/实现——全库是否存在需仓库级核验，本系统不做，"
    "也不得把已在证据中出现过的文件写成缺失。"
