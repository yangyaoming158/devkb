...
**终局复核走了两轮才 `PASS`，中间挨了一次 `RETURN_TO_DESIGN`。** 判回来的理由不是代码有问题——审查者逐字确认「生产代码与测试均无需修改」——是我在定向修复里给的**因果说明本身是错的**：我写「只给一条 evidence 时 claim 被移除、走旧的『full 终态无结构化 claim』规则」。实测探针打脸：`failed_claim_idx` 是空集，claim 根本没被移除；越界的 `[E4]` 只记一条 L0 错误、不指向任何 claim（`verification.py:96-97`），真正起作用的是 `verification.passed=False` → `full_candidate` 不成立（`nodes.py:1355-1357`）→ **mode 从未升到 `full`，那条旧规则一次都没执行过**。

**这是同一个任务里第三次同型失实。**另两次是「修根因链第 1 环会把 2–5 环带好」（实际绕过第 2 环加终态兜底）和 U10「对每个 succeeded 行调 `finalize_consistency`」（落盘 c13/e07 是 `policy_refusal`，送进去必 KeyError——**与我自己在同一份文档里冻结的三态输入域直接矛盾**）。三次的共同点：在 packet 散文里写一句「因为 A 所以 B」，听起来合理、指向真实存在的代码分支、**从没跑过任何东西去验**。测试全绿门禁全过，因为这些句子不在任何机器检查的覆盖面里。普通任务的一次定向修复配额早用完了，所以第三次只能判回设计。登记 `WF-P3-09`：**每条机制断言旁必须附一行可复现的探针输出，或标注「未验证的假设」。**

重新前审 `PLAN_APPROVED` 时又掉出一条同族的小的：packet 三处把引文写成 `verification.py:93-94`，实际在 `:96`——我是从 `sed -n '88,104p'` 的窗口起点数的，偏了 3 行。全仓三处（packet 2 + 清单 1），我第一次 grep 只看了 packet，少算一处。**连行号也属于「说了但没跑过」**，同一条纪律要盖住的东西。

`PASS` 拿到了，含义只有这么多：**正文命中封闭表的 `full` 回答会保守降为 `partial` 并附能力边界声明**。表外措辞照漏（U3 冻结的 fail-open），原否定句照旧留在正文里（U8 反向断言）。审查者还点了一件我该记的：判 `PASS` 那一刻，`## 验收标准` 17 条**物理上全是 `- [ ]`**——`WF-P3-01` 说了一年的老问题，`workflow_guard.py` 至今不校验复选框。现在补勾了，但补勾发生在 `PASS` 之后，不能写成「复核时已勾清」。

**这条规则对真实 Gate 有没有用，现在一个字都说不了。**要等 R-e、R-f 都关掉，第三次真实 dev 跑出来才知道。

## T31.2R-e generate 失败的评测归因（2026-08-08）

**这个任务的第一件事是承认它原来的定义是错的。**

2026-08-07 裁决把 R-e 定为「引用选择缺陷，证据都召回了」。起草 packet 查事实时打开落盘报告：c06/c07/c08/c12 的 `citations` 和 `claims` 全是 0，正文是冻结默认值，`warning_details` 末尾清一色 `generate:1:default_applied`，`llm_calls` 全部触顶 6。**模型根本没走到「选哪条来引用」那一步**——generate 的两次尝试都没拿回合规结构化输出。跨两轮 40 个题次数下来，「generate 成功交付、必需证据在证据集里却零引用」出现 **0 次**。原任务要修的缺陷，现有数据上一个实例都没有。

按勾选纪律停下、登记偏差、等裁决。用户裁定改成评测归因：把「未执行 / 回落默认值 / 交付」在报告里分开，让第三次 dev 能读出失败属于哪一类。**明确接受四题仍不会变绿**——判据层修不了行为层。

**然后我自己的第一版设计也被自己的探针推翻了。**本来打算只用 `warning_details` 判三态：「没有 `node=="generate"` 条目 ⇒ generate 成功」。跑了才发现**成功的 generate 压根不发 warning**，按那条规则 08-05 会把 16 题、08-07 把 13 题误判成「未执行」，其中包括交付了 7 条引用的 c01。改成双来源才对：`trace.steps` 判执行、`warning_details` 判回落。

**前审又抓了两条同族的。**一条 P1：我给变形检验 M4 写的目标是 U6，而 U6 用的是 `delivered` 行——M4 短路的是 `fell_back` 行，**打不中**。一个对 c06 这类行开豁免的实现能大摇大摆穿过我写的文字合同。另一条 P2 更实质：`evidence_paths_from_trace` 只从 retrieve 步的 `output_summary` 取路径，我那个「只带节点名」的 trace **产不出** `retrieved_not_cited`，U6 按原写法根本跑不起来。

三次（含前审两条）都是同一个形状：**写下一条听起来合理的断言，没跑任何东西去验它。**`WF-P3-09` 说的就是这个。区别只在这回全被拦在实现之前——第一条是我自己的探针拦的，后两条是前审拦的。代价是一轮修订，不是一次返工。

顺带两处更正：U9 原写「`":default_applied"` 在 `src/devkb/` 内恰好三处、无第二份字面量」——`nodes.py:781` 和 `trace.py:42` 本来就各有一份，改名后标识符也不止三次，两个数都是编的；U11 原写「禁用措辞扫全篇」——`render_contract_markdown` 固定输出里的「`None`/未判定不通过——方向 fail-closed」含「通过」，全篇扫必红，而唯一的「修法」是改写一句本来正确的诚实文案去迁就测试。**那句零改动，扫描面收窄到两条新增行。**

**第二轮前审判 `RETURN_TO_DESIGN`，同型第三次。**这次挨得不冤。

第一轮前审给了 1×P1 + 4×P2，我做了 Stage 4 允许的唯一一次修订。P1 是变形检验 M4 的目标写错了——M4 短路的是 `fell_back` 行，而我给它指定的 U6 用的是 `delivered` 行，**打不中**；一个专门豁免 c06 这类行的实现能大摇大摆穿过我写的文字合同。修订时我给 U6 换了个种子：落盘 08-07 的 c01，理由是"它 generate 成功交付了 6 条引用"。

第二轮当场驳回：**c01 的 `retrieved_not_cited` 是空的**（`cited=1`、`rnc=[]`、`never_retrieved=1`），逐题 `contract_expectations_met` 实际是 `True`，跟 U6 要的 `False` 正相反。用它根本产不出 U6 的前提。

最难看的地方在这儿：**那个数字就在我自己第一次探针的输出里。**我早先跑过一张全 40 行的表，`c01` 那行清清楚楚写着 `cited=1 rnc=0 nr=1`。我挑种子时只记得"c01 交付成功"，没回头读自己打印过的 rnc 列。

还有一条 P2 也成立：我那个「只带节点名的 trace」——`evidence_paths_from_trace` 只从 retrieve 步的 `output_summary` 取路径，节点名轨迹会让每个必需项都落 `never_retrieved`，`retrieved_not_cited` 永远是空。U6/U10 按原写法**跑都跑不起来**。

三次同型（起草期自查 1 次 + 前审 2 次）的共同点：**写下一条关于"输入长什么样"的断言，没执行任何东西去核。**`WF-P3-09` 原文记的是"因果说明必须有探针"，这三次是"输入事实"，同族但差一格。已把适用面补记进那条 backlog。

**重新设计里最有价值的是审查者给的那个替代方案**：别去枚举豁免形状，改成**变异不变量**——取同一条真实失败行，只改 generate 归因信号造五个变体（`fell_back` / `not_executed` / `delivered` / `budget_exhausted` / `request_failed`），断言 12 个 Gate 键**逐键相同**。任何形状的豁免都会让某两个变体分叉，当场转红，不需要我事先想全所有形状。
...
## 2026-08-09 T31.2R-e 第四轮前审 PLAN_REVISION_REQUIRED——换装置的方向对了，但"能证边界"还是说大了

第四轮头一次有正面抽查结论：A 组两轮分布、C 组基线 `False`、四种豁免 `False→True`、对照组，审查者都自己跑过并确认可复现，也认可 C1 确实把"手写 12 键 dict 写错"那个载体消掉了。

但仍有 1×P1 + 3×P2。四条我逐条复跑，全成立：

- **R4-01（P1）** 我把 C1 叫"完整值域"，还拿它证"12 键取值零变化"。两处都说大了。`outcome_kinds` 是计数字典，值域根本无限，24 种是**代表**不是穷举；`executed`/`fell_back`/`attempt_outcomes` 我也没独立取值。更要命的是**自算基线只能证样本之间相对不变，证不了相对改动前的绝对不变**——而且实现落地后 `assemble_row` 会自动给每行加 `generation`，那时 `_scored` 产出的行本身就带这个键，基线不显式剔除就退化成"注入 vs 注入"。跑了一下确认：当前 `_scored` 行**不含** `generation`，所以现在看着没事，实现后才会踩。还有 C2——我写的是"另比较一次坏结果"，**平行比较通过不能证明 C1 那条断言会失败**。
- **R4-02（P2）** B1 我写"落 `other` 且四个具体类别计数全 0"。`other` 本身就是四类之一，这俩互斥，预期根本不可同时满足，M3 也就没有可信目标。实测是 `{"other": 1}`。
- **R4-03（P2）** 新增的 `fell_back_outcome_kinds` **没有任何断言在守**：A2 只测逐题 scorer，C4 只测四状态分栏，D1 的默认 C 行回落计数全零。聚合层写死返回 `{}` 也能全绿。
- **R4-04（P2）** 我写"当前生产产 20 键"，实跑是 **16**。20 是我上一轮用正则扫 `score_evidence_selection` 的 `return` 源码文本数出来的，**没跑真实装配**。定性结论不受影响，但这数字本身就违反了我自己上一轮定的规矩。

修订按最小边界做完了，最有价值的是 R4-01 的处置——**把锁死方式拆成三重，各担其责**：绝对兼容性交给 `gates` 计算块（`:1040-1114`）零 diff + 既有回归；C1 只声称是**冻结代表矩阵**、只证相对不变，基线显式剔除 `generation`；C2 改成对四个 mutant **调用同一个 `_assert_gate_invariant`** 并断言它抛 `AssertionError`。原来是一件装置扛三件事，现在三件事各有各的证据来源。

**修订期我自己又抓到两处**，都是同一族：

1. 给 D1 补非零渲染断言时，顺手写了 `{c}=1、{d}=2、{e}=1…`——**那是对尚不存在的聚合字段的预测**，我一个都没跑过，也跑不出来。改成冻结 derivation 关系：渲染值 ≡ C4 已断言过的那个 dict，槽位数字一个都不手写。
2. 想加变形 M10（基线不剔除 `generation`），写到"必须转红"那一栏才发现，它的目标同样要预测实现后哪个 mutant 不再抛——**实现前跑不出来**。改成 C1 内部的结构断言 `assert all("generation" not in r for r in base_rows)`，不需要预测，失效当场炸。

同型错误计到 12 例。但这轮的分布变了：9（M2 数字）、以及上面这两处，都是**提交/送审前自己抓的**；10、11、12 是审查者抓的。规矩确实在起作用，只是还没快到能在写下的瞬间拦住。

本轮唯一一次修订用完了。生产代码仍然零改动，`make preflight` PASS。

## 2026-08-09 T31.2R-e 撤销——五轮前审、生产代码零改动，问题不在哪一轮

第五轮又是 `PLAN_REVISION_REQUIRED`，而修订配额第四轮就用完了。三条我照例先自己核：

**R4-01 那条 P1 是真的，而且是设计层的硬矛盾。**C1 要求「删掉 `generation` 键后调 `aggregate_contract` 算基线」，可实现计划第 4 行又要求聚合层读 `row["generation"]["status"]`。我写了个替身模拟实现后的聚合段，跑出来：

```
① 删 generation 算基线  →  KeyError: 'generation'      基线阶段就炸
② 完整五键规范形状      →  可算，Gate = False
③ gates/sections/return 同在一次调用内，sections 在后 → 基线无法只跑 gates
```

C1 按冻结方案**实现不出来**。另两条也成立：换装置表还留着「遍历完整值域」的旧口径，验收还写着 M1–M8 而变形表已经加到 M9。

**关键在于：这条我上一轮不可能发现。**让矛盾成立的那段聚合代码要等实现才存在。前四轮我把所有能跑的都跑了——落盘分布、自比较装置、有牙检验、16 键、`fnmatchcase` 自匹配、`{"other": 1}`——每一条都实证过。剩下这一条**结构上不可探针**。

于是问题就清楚了，它不在哪一轮：**这个任务的交付物几乎全是测试装置，而装置测的是尚未写出的代码的性质。**packet-first 冻结在交付物是产品行为时是成立的——装置能拿当前生产代码验。装置本身当交付物时，前置审查被要求认证一个它结构上无法认证的对象。五轮真实缺陷、零生产代码，是这个错配的症状，不是审查者苛刻，也不全是我不小心。

加上任务本身的产出从 2026-08-08 裁决起就写明了「这四题仍然不会变绿」——继续投入的边际收益是负的。三条候选（撤销 / 批准例外修订 / 回 Stage 2–3）都得用户裁决，我建议撤销，**用户裁决撤销**。

撤销不等于丢掉。packet 文末冻结了「交接说明」四节：三处未关闭修法（含已验证的修法——基线改用完整五键规范形状，探针实证可算）、已验证的归因数据（两轮 `10/8/2/0` 与 `6/9/5/0`、08-07 五道 `fell_back` 题的失败 code、`delivered ∧ rnc = 0`）、U3.2 可直接粘贴的模板、治根方向（`OpenAICompatLLM` 未设 `max_tokens` 配 `timeout_s=60`，c06 实测 `tokens_out=38905`/`latency_ms=241736`）。U3.2 的判据也增列了归因小节并交叉引用。

归因这件事本身其实十分钟就能做完——`warning_details` 里四题的 code 肉眼可读，U3.2 本来就要逐 run 打开轨迹。**为一个十分钟的人工动作建了五轮的机器装置**，这是这次最该记住的一条。
...
## 2026-08-09 · T32.1 + T32.2 收尾：README 换成 P1.5 口径，验收总清单逐条核到底

T31.2 已经没有"验收"这个动作可做——它的完成判据第 1 条是**测量结果**（13 题达到契约预期），不是可交付物。第三次 dev 跑完是 `all_hard_gates_passed=False`，那就只能不勾选。唯一能让它变绿的路径（再做行为层修复 → 跑第四次 dev）是用户裁决关掉的。所以直接进 T32。

**T32.1（README）。** 最容易犯的错是把"我们实现了 X"写成"我们具备 X 能力"。这次踩到一次：我在 `not_found` 四分类表里给 `forbidden_or_unavailable_by_policy` 标了 ✅，写"由 policy-refusal 终态产出"。翻本清单核对 T27.1 的验收记录时才看见那行末尾明写着——「`forbidden_or_unavailable_by_policy` 在 P1.5 结束时仍是『枚举齐备但无代码产出』的类别，其语义由 `mode=policy_refusal` 终态承载，**T32.1 的 README 不得宣称该类别有明细项输出**」。半年前的自己给半年后的自己留了一道拦截线，正好拦住。改成 ❌ 无明细项输出 + 语义由终态承载。

这件事的教训不是"我粗心"，而是**验收记录里那些"将来别这么写"的约束，只有在写将来那份文档时去翻才有用**——它们不会自己跳出来。

其余改动里两处是过期措辞，不改就是留假话：①"Hybrid 是否上线待 **P2** 检索改进后重议"——ADR-0009/0010 之后检索改进（RT-06）明确归 P1.6，规格 §13 写着；②安全边界句只说 P1 无鉴权，P1.5 同样无鉴权。另外补了一条我一直觉得该说但 README 从没说的事实：**P1.5 的契约评测跑在另一个陌生仓库上**（`rag-kb-p1-20260722`，183 文档），mini-mall 只用于检索无退化对照。开发语料上的表现本来就不该算数。

「已知限制」从 5 条 P1 口径扩到 10 条 P1.5 口径。新增的几条都是"我们的机制在什么条件下失效"，不是"我们还没做什么"：否定词表是**封闭表匹配不是语义判定**（措辞在表外就拦不住，已知 fail-open）、policy 三张表同样是闭表（漏杀存在，命中也只是词法事实不是"恶意"判定）、`max_tokens` 未设是 `invalid_structured_output` 的**假设**成因而非已验证结论。最后一条尤其要写清"未验证"——它现在是个假设，写成结论就是又一句漂亮的假话。

**T32.2（验收总清单）。** §15 十条逐条核，结论是 **5 达成 / 1 部分 / 4 未达成**。真正花时间的是决定哪些**不能勾**：

- 第 3 条 `zero_full_without_required_evidence = True`，看着是绿的。但本轮 20 题终态是 partial 14 / refusal 4 / policy_refusal 2 / **full 0**——**零 full 意味着这个键根本没被施压**。一个从未被触发过的守卫返回 True，不构成"守卫有效"的证据。不勾。
- 第 2 条更微妙：机器可判定部分 `not_found_decidable_ok = 104/104` 全绿，已知路径否定扫描 142 个可判定语段里只命中 2 段、且两段都是同语段共现误报（正文说的是"抛 BusinessException('知识库不存在')"，跟证据路径 `BusinessException.java` 撞在一句里）。但 `not_found_proposition_undecided = 104`——**104 条命题级全部判不了**，按设计全数路由 U3.2 人工复核。**这条的达成与否不归我判，归人工复核。**代人裁决就是越权。
- 第 9 条 holdout 从未运行。**未运行不等于达成**。

顺手核了偏差记录 43 行，用户裁决列全部非空，没有悬空项。

**收尾定性**（写进清单供 T32.5 裁决用）：未达成的 4 条里**没有一条是"答错了"**——第 1 条是判不了、第 2 条待人工、第 3 条是没被施压的空真、第 9 条是裁决不跑。P1.5 建的是一套**会说"我判不了"的测量装置**，它现在正对着自己说这句话。这和"装置失效"是两回事，但也**不构成通过**。

门禁：`make ci` **1351 passed**（ruff/pyright 0 错）、`make eval-ci` **615 + 65 passed**。README 里每个数字都是程序从落盘报告读出来核过的，没有一个是从记忆里抄的。

**T32.4 追记（同日）**：`p1.5` 推送 origin（89 commits，`ccef364..05b1cc4`），GitHub Actions run 31313520736 **completed / success**，`make ci` 与 `make eval-ci` 两步均绿。特意核了 `head_sha`——`05b1cc4e8136…` 与本地 `git rev-parse HEAD` 逐字相同，判据写的是"不以较早提交代替"，那就得真的比对而不是看见个绿勾就算数。远端 runner 环境与本地不同（境外直连 PyPI、不装 ml 组、Postgres 走 service 容器），所以这是一次**独立复现**，不是本地结果的回声。验收总清单第 10 条据此由「部分达成」转为达成，十条里达成数 5 → 6。

## 2026-08-09 · T32.5：P1.5 以 Gate 未通过状态冻结

用户确认了冻结。我把这次勾选的含义在清单里写死成一句话：**「阶段到此为止、状态如实冻结」，不含「验收通过」「判据达成」「可对外宣称能力」任何一层意思。**写这句话是因为半年后翻记录的人（很可能是我自己）看到 T32.5 打了勾，第一反应一定是"P1.5 过了"。

冻结时的真实状态原样留档：§15 **6 达成 / 4 未达成**，`all_hard_gates_passed = False`。**4 条未达成一律不补勾、不改判据、不重跑求好数**——理由不是"守规矩"，而是这 4 条恰好是 P1.6 的题目本身。第 1 条"判不了"要的是命题级判定能力，第 2 条 104 条命题级未判定要的是仓库级 inventory，这两样规格 §13 明写归 P1.6。把它们涂绿等于把 P1.6 的起点抹掉。

同步改了 `CLAUDE.md` 的阶段行与范围纪律：阶段从"P1.5 进行中"改成"P1.5 已冻结（Gate 未通过）/ P1.6 尚未开工"，并明确**冻结解锁的是 P1.6 三份文档的创建权，不是实现权**——文档写成并冻结前，任何 P1.6 行为仍禁止实现。范围纪律里加了一句"不得回头补勾未达成项、不得为凑绿改判据或重跑评测"，把这次的自律变成对后来者的规则。

仍然封着的：`contract_holdout.jsonl`，三轮 dev `holdout_accessed=false`，动它仍要走 U3.3 授权。仍然待人工的：U3.2，它不阻塞冻结，但验收第 2 条的终值以它为准。

**P1.5 到此结束。一个诚实的失败结论，比一个凑出来的绿灯有用——前者告诉你下一步该做什么，后者只告诉你别再看了。**

## 2026-08-09 · U3.2 待填复核单：把机器能做的全做掉，把机器不能做的原样留白

P1.5 冻结后剩两项在用户手上：U3.2（人工复核）和 U3.3（holdout 授权）。U3.3 我建议不跑——系统已以 Gate 未通过冻结，holdout 是最后一份未污染的数据，现在跑等于用最后的干净数据去测一个已知不通过的系统，测完数据就烧了。留给 P1.6。

U3.2 则值得把机械部分全做掉。**先查了一件决定性的事：语料源还在不在。** `rag知识库项目 @9bb79cd` 与报告 manifest 逐条 sha256 比对——**183/183 逐字节相同**。这意味着每条引用的 `rel_path:行号` 都能翻回真实源码，且内容就是系统当时看到的那份。有了这个，复核就从"逐个开文件数行号"变成"看并排的两栏"。

生成的 `evalsets/reports/p1.5-manual-check.md`（1779 行）里，每条 claim 展开成：断言 → 绑定的 `[E#]` → 模型给出的引文（已过 L1）→ 该 E# 指向的 `rel_path:起止行` → **从磁盘读出的真实源码**（`>` 标记锚定行段、上下各 2 行）。32 条引用全部解析成功，零缺失、零哈希不符、零 L0 越界残留。

**102 处结论格全部留空，这是刻意的。**如果我把判断预填进去，复核就退化成盖章——而 U3.2 存在的唯一理由就是机器判不了 L2 语义支持度。我能摆出证据，不能替人下结论。这条我写进了文件抬头。

104 条 not_found 按 `basis` 分了组，并标出轻重：`corpus_index` 那 **10 条必须逐条看**——它们正是"系统发现该路径其实在索引里、于是标注成『本轮未召回的证据缺口』而不是『不存在』"的地方，验收第 2 条问的就是这个，而这 10 条恰好证明机制在工作。`static_suffix_rule` 13 条是纯后缀规则判的，抽查即可。这样分组之后，需要人真正动脑的从 104 条压到十几条。

**过程里踩的两个坑都值得记：**

一、语料里有 `architecture.md` 的 mermaid 块，源码切片本身含 ``` 围栏，写死三反引号会把外层代码块提前截断——生成出来的第一版有 22 处断裂。改成按内容里最长反引号串动态算围栏长度。**教训是：任何"把外部文本嵌进 markdown"的代码都得假设外部文本里有围栏。**

二、把生成器收进 `scripts/` 时被 ruff E501 拦了 22 处。自动拆行脚本连着失败两轮，原因很蠢也很典型：**ruff 的 line-length 按东亚宽度算（中文字符占 2 列），我的预算用的是 `len()`（码点数）**，所以拆完还是超。改用 `east_asian_width` 计宽后一轮过。同一个坑——"以为自己在数同一个东西，其实不是"——这个项目里已经出现过好几种变体了。

`full` 样本有个判据冲突，按纪律提请裁决了：判据要求覆盖一个 `full`，而 **P1.5 三轮只产生过一个 full——run2 的 e06，正是后来被 R-d 判为不诚实的那题**（肯定语气断言"未出现 Kafka/RabbitMQ"）。最终轮零 full。用户裁定收它作样本并如实标注。我在两处逐字写明它来自 run2、是全程唯一的 full、是 R-d 的起因案例——同一个事实在验收第 3 条那里是"空真、未被施压"的依据，两处口径必须一致，不能一边当样本一边装作没这回事。

门禁：`make lint` 全绿、`make ci` **1351 passed**。

## 2026-08-09 · U3.2 复核结果：人工在机器 104/104 全绿的地方找出了编造的类名

用户把复核单填完了，102 个格子全填。结论表五行里有两行是「发现问题」——**这份复核没有变成盖章**，这是它最大的价值。

**三条发现，我逐条独立核验，全部成立，而且有两条比原述更严重。**

一、**编造类名。** 用户写「c07 出现无证据的 `KnowledgeBaseService`/`ConversationService`，e06 出现无证据的 `AsyncConfig`」。我去语料源仓库 `find -name '<X>.java'` 加 `grep -rl 'class <X>'` 查了——**三个类在仓库里根本不存在**。所以不是"存在但没被引用"，是模型编的。这直接违反规格 §15 第 2 条「`not_found` 中不出现未经证据支持的路径/类名/字段」。

二、**refs 绑错目标，而且绑错之后措辞更可信了。** c07 把「会话删除的生产实现」绑到 `domain/Conversation.java`（一个领域实体），实际实现在 `ConversationController` 和 `RagService`。根因在代码里坐实了：`refs` 由 `iter_path_tokens(text)` + `path_matches_token`（`not_found.py:379/393`）从**模型自己写的缺口句子**抽路径 token 再与索引比对，而 `path_matches_token` **只判文件身份、不判命题**——这条边界该函数 `:461` 的 docstring 自己写着。「会话删除（**Conversation** delete）的生产实现」→ token `Conversation` → 命中 `domain/Conversation.java`。

**连带效应比绑错本身更糟**：`basis` 因此被标成 `corpus_index`，文案变成「该路径已在当前项目索引中（状态 active），本轮未被召回，属当前证据缺口」——**一句关于错误文件的、听起来经过核验的断言**。绑错反而让它更像真的。

**而这正是 `T31.2R-f` 要修的缺陷**——2026-08-07 立项、2026-08-09 被裁决跳过。用户在不知道有这个任务的情况下独立挖出来了。这比我当初写的任何立项理由都更能说明 R-f 有必要。

三、**claims 未进入正文。** 用户判 c06 的 claims 11–15。我跑了个前缀探针：16 条里只有 5 条前缀逐字出现在正文。**缺口可能更宽**——但前缀子串探针分不出"改写了"和"没写"，所以我把探针结果记为佐证，**不用它覆盖用户的阅读结论**。这个区分很重要：机器给的是更弱的信号，不能因为数字更大就当更准。

**这三条里我最该记住的一件事**：评测层 `not_found_decidable_ok = 104 / 104` **全绿**，而人工在同一批 104 条里找出了编造类名。因为机器可判定子集只覆盖"结构/枚举/静态后缀/已知路径措辞"，"这个类名在仓库里存不存在"属命题级，`not_found_proposition_undecided = 104` 全数路由人工。**「判不了就路由人工」这条设计今天兑现了。**如果当初图省事把命题级也算进"可判定"，104/104 全绿会把编造类名彻底埋掉。

**一个我必须自己认的错。** 我在 T32.2 把验收第 4 条标成了 ✅ 达成，依据是机器 Gate `consistency_all_true = True`。用户第 4 行填的是"否"。两边都没算错——机器 Gate 查的是 T25.1 那套具名规则（full 带缺口 / full 零 claim / 命中否定标记降级），**不包含"claims 在结构化输出里、正文里没有"**。但判据字面写的就是「`mode`、**正文**、`claims`、`not_found`、`limitations` 一致」，已知不一致还挂着 ✅，与诚实汇报纪律直接冲突。提请裁决后降级，统计由 **6 达成 / 4 未达成** 改为 **5 / 5**。

这里有个原则我顺手写进了 T32.5：**冻结记录只允许朝不利方向如实修正，不允许朝有利方向自行修订。**本次两处修订（第 2 条定为未达成、第 4 条降级）都朝不利方向，而冻结结论是"Gate 未通过"——新证据只会让它更不通过，逻辑上不可能翻绿，所以 T32.5 不重开。反过来，如果哪天出现让某条由未达成转达成的证据，**必须**重新提请裁决。这是"不得回头补勾"的另一面。

三条发现全部转 `docs/backlog.md`（`U32-P16-01/02/03`）作 P1.6 输入，**P1.5 冻结后不回头修**。README「已知限制」相应从 10 条扩到 12 条，新增的两条就是编造类名和 claims 未进正文——CLAUDE.md 写着"引用/回答机制的已知限制写入 README 而非掩盖"，这正是该写进去的东西。
