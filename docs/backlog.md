...
| 2026-07-20 | Fable 5 | partial 终态被低质量 `not_found` 触发（generate 输出占位/否定式条目仍降级为 partial） | U2.2 材料准备时发现：8 个 partial 中 q07 的 not_found 为占位式「未覆盖方面」、q05 为语义相反的「无其他未覆盖的方面。」、q04 的 not_found 为空却仍判 partial，三者本可为 full。不破坏 §5.2 任何硬项（partial 是合法终态、L0/L1/预算/终态全过），但使 mode 分布偏向 partial，也让 full/partial 的对外表述偏保守。可选修法：generate 后对 not_found 做确定性清洗（空串/复述字段名/否定式条目丢弃），或让 finalize 在 not_found 为空时不判 partial | 待裁决（P1 不修：改判定逻辑等于在 17 题 dev 集上调参，违反 Evaluation-v1 纪律） |
