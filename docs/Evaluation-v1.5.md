...
- **两栏预期（U3.1 预登记规则）**：§21.1 原判据混有依赖 P1.6 能力（多语言摄取、inventory、检索改进）的要求，而 P1.5 不改检索与摄取，故每题必须登记两栏：
  ...
  - 现在即冻结的 P1.5 契约预期：题 3（Vue/TS）= `unsupported_or_not_ingested` + 静态覆盖披露，不得表述为"仓库无源码"；题 10 的迁移方面 = `unsupported_or_not_ingested`，与已索引 Java 的证据缺口分类区分；题 11 = 诚实能力边界声明（与 §21.1 既有 fallback 句一致）；**其 `expected_mode_p15` 于 T31.2R-b 由 `partial` 改为条件式 `partial_or_refusal`**（用户 2026-08-05 裁决）——该题冻结散文原本就写着「若 PROGRESS/plans 未召回则 refusal-with-boundary 亦可接受」，机器字段与散文自相矛盾。**这是放宽，不得读作题 11 合格**：条件性由 `contract_expectations_met` 的第二个合取项 `not retrieved_not_cited` 承担，`PROGRESS.md` 已召回却未被直接引用时**仍判失败**，`p16_target` 的三态闭合要求不变；**［T31.2R-c 收紧，2026-08-07］条件式规则的另一半「未被召回则以 `missing_from_current_evidence` 呈现」此前没有任何硬键在守——一道必需证据全未召回的题，第二个合取项空真成立，整题只剩 `mode_matches` 在判（T31.2 第二次 dev 的 c02 即如此被计入达标）。现由第三个合取项承担：每个 `never_retrieved` 必需项都必须被一条合法 category 的 `not_found` 明细在 `refs` 里结构性引用，逐题判定改为三态——确定失败 `False`、归不了属（明细 `refs` 为空）`None`、全部可判定通过才 `True`，`None` 不算通过。**注意措辞边界：`referenced` 只证明「合法类别的 refs 中存在该路径」，不证明该缺口陈述在说这一项、不证明该路径真实存在——命题真伪逐项记 `proposition_undecided` 并路由 U3.2，不进 Gate。**这是收紧而非放宽：新合取项只能把逐题结果由 `True` 降为 `None`/`False`，永不反向。］**；题 12（Controller 穷举）= 诚实能力边界声明（不假拒答、不假断言穷举或不存在）。
