...
Agentic RAG 软件项目知识助手（面向求职展示）。P0 已于 2026-07-16 复验通过；P1 已于 2026-07-22 验收关闭；**P1.5 真实仓库可靠性/诚实加固（ADR-010）已于 2026-08-09 以「Gate 未通过」状态如实冻结**——退出 Gate（规格 §15）10 条中 6 达成 / 4 未达成，`all_hard_gates_passed = False`（详见 `docs/P1.5任务清单.md` 的「P1.5 验收总清单」与 T32.5）。**冻结 ≠ 通过**：不得对外宣称 P1.5 判据达成，也不得回头补勾或改判据。当前阶段：**P1.6 尚未开工**（冻结已解锁其实现规格/任务清单与 D6 窄修订 ADR 的创建，但文档尚未创建，创建前不得实现任何 P1.6 行为）。
...
- **范围纪律**：清单外的想法/优化写入 `docs/backlog.md`，不得直接实现。**P1.5 已冻结**，其清单只作历史记录与 P1.6 起点，**不得回头补勾未达成项、不得为凑绿改判据或重跑评测**；仍开放的只有 U3.2（人工复核）与 U3.3（holdout 授权，holdout 至今封存）。**P1.6 的实现规格/任务清单与 D6 窄修订 ADR 现已解锁可创建，但在这三份文档写成并冻结之前，禁止实现或细化任何 P1.6 行为**（含多语言摄取、inventory、动态工具调用、检索改进）；P1.6 关闭后才创建 P2 文档，当前禁止实现或细化 P2/P3。P0/P1 兼容性回归继续是硬约束，但不得借兼容之名增加清单外功能。
